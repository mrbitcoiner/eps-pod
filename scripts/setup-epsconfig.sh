...
readonly WALLET_CONFIG="${RELDIR}/../config.ini"
readonly EPS_CONFIG="${RELDIR}/../data/config.ini"
####################
source "${RELDIR}/../.env"
####################
setup() {
	cat "${WALLET_CONFIG}" > "${EPS_CONFIG}"
	cat << EOF >> "${EPS_CONFIG}"
[bitcoin-rpc]
...
wallet_filename = ${BITCOIND_EPS_WALLET_NAME}
